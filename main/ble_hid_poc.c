...
// One combined report map (single HID service, as HOGP expects): keyboard
// (report ID 1) + mouse (report ID 2) + consumer control (report ID 3).
// Separate HID services per report are non-standard - Android only exposes
// the first one, so mouse/consumer reports would never reach the phone.
const unsigned char comboReportMap[] = {
    ...
    0x95, 0x05,        //   Report Count (5) - key codes
    0x75, 0x08,        //   Report Size (8)
    0x15, 0x00,        //   Logical Minimum (0)
...
// HID Report Maps - a single combined map in one HID service
static esp_hid_raw_report_map_t ble_report_maps[] = {
    { .data = comboReportMap, .len = sizeof(comboReportMap) },
};
