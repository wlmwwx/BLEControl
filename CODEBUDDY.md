...
`main/CMakeLists.txt` compiles exactly two files; anything else in `main/` is not in the build.

| File | Role |
| --- | --- |
| `ble_hid_poc.c` (902 lines) | Everything: HID report descriptors, WiFi AP, HTTP server + all endpoint handlers, command queue, UART console, `app_main` |
| `esp_hid_gap.c` / `.h` | NimBLE GAP layer only — BT controller init, advertising payload, SM pairing config, GAP event handler |
| `idf_component.yml` | Component manager manifest (pulls `espressif/led_strip`) |
| `Kconfig.projbuild` | Defines the `Example Configuration` menu (LED type/GPIO/period) |

The split matters: **all HTTP→HID translation lives in `ble_hid_poc.c`**, and `esp_hid_gap.c` is the only place that touches NimBLE GAP APIs directly.

### Startup sequence (`app_main`, ble_hid_poc.c:837)

1. `nvs_flash_init` (erases NVS and retries if no free pages / version mismatch)
2. `wifi_init()` — brings up the WiFi AP
3. `http_server_start()` — HTTP server comes up **before** BLE
...
WiFi runs in **AP mode** (not STA), hardcoded in `ble_hid_poc.c:328`: SSID `BTControl`, password `12345678`, max 3 clients, WPA/WPA2-PSK. The device therefore sits at `192.168.4.1` and the controller PC must join the ESP32's network.

`http_server_start()` (line 661) registers 14 URIs on the default port (80). **`config.max_uri_handlers` is set to 20** — the esp_http_server default is 8, which would silently drop the last 6 endpoints with `httpd_register_uri_handler: no slots left`. Endpoints are **unprefixed** — the `/api/v1` prefix in the PRD is not implemented:
...
`sdk/python/btcontrol.py` is a thin `requests` wrapper, default host `192.168.4.1`, matching the unprefixed endpoints above: `type`, `key`, `move`, `click`, `double_click`, `scroll`, `drag`, `press_mouse`, `release_mouse`, `queue_add`, `queue_exec`, plus consumer helpers (`volume_up`, `mute`, `play`, `next_track`, …) and composite helpers (`select_all`, `copy`, `paste`, `undo`, `save`). `tap`, `swipe`, and `move_to` exist in the SDK but have **no firmware endpoints** — they are PRD-forward and will 404. Keep SDK and firmware endpoint lists in sync when adding routes.

## Where the project is headed

`docs/prd.md` defines phases 0–8. Git history tracks the climb: phase 0 (BLE HID PoC) → 1 (keyboard) → 2 (mouse) → 3/Phase 1 (WiFi AP + HTTP API) → 4 (command queue + Python SDK) → 5 (consumer control) → 8 (info endpoint), tagged `v0.1-phase0-proof-of-concept` … `v1.0-production`, with uncommitted fixes on top (BT controller init, NimBLE host task, consumer descriptor, HID instance limit, HTTP handler slots). The HEAD commit "Fix: Enable BLE advertising" is broken as committed — BLE does not initialize at all without those fixes.

Deferred by the PRD and **not** in the firmware: `/api/v1` URL prefix, Bearer-token auth, touch/digitizer/swipe, absolute `move_to`, STA-mode provisioning with mDNS (`btcontrol.local`), WebSocket transport, OTA, workflows.
